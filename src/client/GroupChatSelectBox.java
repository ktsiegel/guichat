...
/**
 * A GroupChatSelectBox is a GUI that lets the user select whom he/she wants to
 * have in a group chat.
 */

public class GroupChatSelectBox extends JFrame {
    private static final long serialVersionUID = 66723252538538343L; // randomly
                                                                     // generated
                                                                     // serial
                                                                     // ID
    ...
    private JButton createChatButton; // when clicked, creates a group chat with
                                      // the selected users.
    private Set<JLabel> selected;

    /**
     * Creates a GroupChatSelectBox associated with a chat client model.
     * 
     * @param model
     *            the ChatClientModel associated with this GroupChatSelectBox.
     */
    public GroupChatSelectBox(ChatClientModel model) {
        ...
        createChatButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for (JLabel label : selected) { // add all selected users to a
                                                // new set of group chatters
                    chatters.add(new User(label.getText()));
                }
                chatters.add(clientModel.getUser()); // add yourself to the
                                                     // group chat as well
                if (chatters.size() > 1) {
        ...
        for (User user : clientModel.getUsers()) {
            if (!user.equals(clientModel.getUser())) {
                ...
                URL url = cl.getResource("icons/avatar" + user.getAvatar()
                        + ".png");
    ...
    private void createGroupLayout() {
        ...
        h.addComponent(createChatButton, GroupLayout.DEFAULT_SIZE,
                GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE);
    ...
    /**
     * Checks whether a user (represented by a label) is contained in the stored
     * list of selected user labels.
     * 
     * @param label
     *            The label that is checked.
     * @return whether or not the list of selected user labels contains the user
     *         label.
     */
     ...
     * 
     * @param label
     *            The label that should be added to the list of selected users'
     *            labels.
     */
     ...
     * 
     * @param label
     *            The label that should be removed from the list of selected
     *            users' labels.
     */
